<?php 
    ...
    if(!empty($comp_name)) {
          ...
          $sql = "INSERT INTO work_exp(tin_num,comp_name,status,designation,salary,branch,exp_date_from,";
          $sql .= "exp_date_to,leave_abs,leave_abs_date) ";
          $sql .= "VALUES('$tin_num','$comp_name','$status','$designation','$salary','$branch',";
          $sql .= "'$exp_date_from','$exp_date_to','$leave_abs','$leave_abs_date') ";

          if($db->query($sql)) {
            $query = "SELECT * FROM work_exp WHERE tin_num='$tin_num' ORDER BY id DESC LIMIT 1";
            $result = $db->query($query);
            $work_row = $result->fetch_array();
